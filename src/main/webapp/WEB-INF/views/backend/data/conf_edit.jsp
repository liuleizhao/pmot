...
<body>
	...
	<form class="classA mui-input-group z_99" action="${ctx }/backend/data/conf/edit" method="POST" name="form" id="editFrom">
		...
		<table cellpadding="8" cellspacing="1">
			<tr>
				...
				<td class="text">
					<div class="mui-input-row">
						<input type="text" name="tableName" readonly="readonly" value="${export.tableName }"/>
					</div>
				</td>
				...
				<div class="mui-input-row">
						<select name="state" id="state">
							<c:forEach items="${stateList }" var="entity" varStatus="status">
								<option value="${entity }"<c:if test="${entity eq export.state }">selected="selected"</c:if>>${entity.description }</option>
							</c:forEach>
						</select>
			...
			<tr>
				...
				<td class="text" colspan="3" style="height: 80px;">
					<div class="mui-input-row" style="height: 80px;">
						<textarea id="field" name="field" readonly="readonly" placeholder="不选择字段则导出全部信息..." style="width: 80%;border: 1px solid #CCC;margin: 8px 10px;">${export.field }</textarea>
			...
			<tr>
				...
				<td class="text" colspan="3">
					<div class="mui-input-row" style="height: 80px;">
						<textarea id="description" name="description" placeholder="简单描述一下..." style="width: 80%;border: 1px solid #CCC;margin: 8px 10px;">${export.description }</textarea>
					</div>
				</td>
			</tr>
		</table>
