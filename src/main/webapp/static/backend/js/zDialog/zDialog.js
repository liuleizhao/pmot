...
parseJsArgument();
jsCtx = scriptOptions.ctx || ctx; //赋值上下文路径
var IMAGESPATH = jsCtx+'/static/backend/images/'; //图片路径配置
        ...
        return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input type="button" class="button blue" style="width:80px;" value="确 定" id="_ButtonOK_' + this.ID + '"/>\
                <input type="button" class="button blue" style="width:80px;" value="取 消" onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
