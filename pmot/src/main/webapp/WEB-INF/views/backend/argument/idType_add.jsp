...
<body>
	...
	<form class="classA mui-input-group" action="${ctx }/backend/argument/idType/add" method="post" name="form" id="form">
		<table cellpadding="8" cellspacing="1">
			<tr>
				<td class="field"><span class="txt-imp">*</span>证件代码</td>
				<td class="text">
					<div class="mui-input-row">
						<input name="code" id="code" type="text" value="${idType.code}"/>
						<span style="color:red;marign-left:20px;display:none;">证件代码已存在</span>  
					</div>
				</td>
		...
		$("#addBtn").click(
			function() {
				try {
					...
					$(this).validate.isNull($("#name").val(),"证件类型名称不能为空");
					//提交表单
